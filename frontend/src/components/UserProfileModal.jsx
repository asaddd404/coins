    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Профиль пользователя">
      {loading ? (
        <div className="p-8 flex justify-center">
          <div className="animate-pulse text-blue-400">Загрузка профиля...</div>
        </div>
      ) : profile ? (
        <div className="p-6">
          <div className="flex flex-col items-center mb-6">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-4xl text-white font-bold mb-4 shadow-lg shadow-blue-500/30 border-4 border-slate-800">
            ...
            <h2 className="text-2xl font-bold text-white text-center flex items-center gap-2">
              ...
              {profile.role === 'teacher' && <Shield className="w-5 h-5 text-cyan-400" />}
            </h2>
          ...
          <div className="space-y-3 bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            <div className="flex items-center gap-3 text-slate-300">
              ...
              {profile.phone ? (
                <a href={profile.whatsapp_url} target="_blank" rel="noreferrer" className="hover:text-cyan-400 transition-colors">
                  {profile.phone}
                </a>
              ) : (
