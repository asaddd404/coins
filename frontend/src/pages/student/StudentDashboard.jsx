    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {sharing ? <Check className="w-4 h-4 text-cyan-400" /> : <Share2 className="w-4 h-4 text-blue-400" />}
      ...
      {data.groups.length === 0 ? (
        <div className="glass-card p-12 flex flex-col items-center justify-center text-center">
          ...
          <Link to="/courses" className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-medium transition-colors shadow-lg shadow-blue-500/20">
            Перейти в каталог
          </Link>
        </div>
      ) : (
            ...
            <div key={g.group_id} className="glass-card overflow-hidden hover:-translate-y-1 transition-transform duration-300">
              <div className="p-6 border-b border-slate-700/50 bg-slate-800/30">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className="text-xs font-bold uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-2 py-1 rounded-md mb-2 inline-block">
                      {g.course_title}
                    </span>
                    <h3 className="text-xl font-bold text-white">{g.group_title}</h3>
                  </div>
                  <Link to={`/courses/${g.course_id || ''}`} className="w-10 h-10 bg-slate-800 hover:bg-blue-500/20 rounded-xl flex items-center justify-center text-slate-400 hover:text-blue-400 transition-colors border border-slate-700">
                ...
                <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/50 glow-indigo">
                  ...
                  <div className="mt-3">
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="text-blue-400 font-bold"><AnimatedNumber value={g.total_xp} /> XP</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5 border border-slate-700">
                      <div className="bg-gradient-to-r from-blue-500 to-violet-500 h-1.5 rounded-full" style={{ width: `${(g.total_xp % 50) / 50 * 100}%` }}></div>
                    </div>
